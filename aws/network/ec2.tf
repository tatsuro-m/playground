...
echo '<!DOCTYPE html>
            ...
            <head>
                <meta charset="UTF-8">
                <title>EC2(public subnet) Web httpd instance(No.2)</title>
            </head>
            <body>
                <h1>EC2(public subnet) Web httpd instance(No.2)</h1>
            </body>
            </html>' > /var/www/html/index.html
systemctl restart httpd.service
EOF
...
resource "aws_instance" "internal2" {
  ami           = "ami-0f89bdd365c3d966d"
  instance_type = "t2.micro"
  subnet_id     = aws_subnet.private_c.id
...
<head>
　  <meta charset="UTF-8">
    <title>EC2(private subnet) Web nginx instance(No.2)</title>
</head>
<body>
    <h1>EC2(private subnet) Web nginx instance(No.2)</h1>
</body>
</html>' > /usr/share/nginx/html/index.html
